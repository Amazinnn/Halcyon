# Focus Desktop
> 当前 Agent/工作流语义以 [ADR-0024](./docs/decisions/ADR-0024-agent-workflow-closure.md) 至 [ADR-0027](./docs/decisions/ADR-0027-workflow-trigger-node-and-weekly-schedule.md) 为准：每个桌宠在设置页固定选择真实 Codex 或 Claude；聊天保持极简并回放当天可见消息；工作流 Agent 只在允许显示结果时向目标 Agent 投递一条带来源的最终消息。
> GitHub：https://github.com/Amazinnn/Halcyon.git（private）

本地专注桌面与 Agent 桌宠系统（Windows 优先，MIT License）。

**当前检查点（2026-08-10）**：Claude Code 与 Codex 都按桌宠常驻；当天可见聊天按“桌宠 × Provider”隔离回放，Focus 重启后的首轮恢复当天 session。聊天移除生命周期噪音，Skill 仅附加到下一条真实消息；工作流有不可持久化的触发节点，并支持间隔、每日和每周计划。浮窗无空位时保持折叠并给出简短提示，release 自动化确认不会重叠；淡蓝标题条的视觉复验与桌面锁仍待人工确认。自动化、重建与真实 Claude/Windows 验收状态见 [最新 Eval](./docs/evals/2026-08-10-window-regression-checkpoint.md)；需求 #79–#85，ADR-0025–0027。
