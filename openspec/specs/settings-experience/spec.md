...
- **WHEN** the next direct Claude response produces thinking or public text
  increments (or a Codex response produces public text deltas)
- **THEN** chat displays those increments before the final reply

#### Scenario: Provider reasoning or tool activity occurs
- **WHEN** a direct conversation produces a workflow event or tool activity
