# Pet Companion Bubble Specification
...
The companion SHALL paginate complete text into sentence-complete pages of at
most four measured lines (whole short paragraphs, one sentence per page for
longer paragraphs), rotate each page every three seconds, fade between pages,
and immediately restart from page one when a new message replaces the current
message. Its host SHALL size to the content (180-340px wide, height by lines).
