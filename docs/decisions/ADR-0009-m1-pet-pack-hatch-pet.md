...
- ???????2026-08-07??SenseNova ????????????????????`multimodal-ai/scripts/sensenova_image_gen.py`?key ??????????????????? 6 ?? ? 2 ??idle/jumping/failed/waiting/running/review????????? 1536?1872 atlas??? 3 ?? idle ???????? validate???? validate_atlas.py ?? `ok: true / 0 errors / 0 warnings`??? `focus-demo-pet`?pet.json + spritesheet.webp????? app data `pet-packs/` ????settings.petPackId??????????????
