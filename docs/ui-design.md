...
1. 组件内 `font-family: inherit` 必须在 `font-size` 之前声明；禁止
   `font: inherit` shorthand 出现在 font-size 之后（会覆盖字号）。
2. 表单行内紧凑下拉（如 Skills）用固定宽度，不受最长选项影响；弹出列表
   仍按内容宽度。
3. 开关行（label + toggle）用 `.toggle-row` 浅分隔线（rgba(255,255,255,0.05)
   1px，与文字留 8px 距离）划分行组。
