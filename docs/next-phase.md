...
- Codex 当前认证状态不影响 Claude 验收；Claude 不可用时保留其原始错误，不得回退 Mock。
...
### v1.12 桌面锁后端（2026-08-08 已实现，手工回归纳入质量检查点，需求 #70，ADR-0023）
...
### Agent 对话与工作流闭合（2026-08-10 已实现，真实 Claude 控制面已验收，需求 #76–#78，ADR-0024）
- 正式桌面路径不自动回退 Mock；聊天极简；工作流固定为统一全量日程列表；仅 `showResult` 的 Agent 节点最终结果可一次性带来源回流到目标 Agent 对话与宠物泡泡。
- **剩余门槛**：打开聊天窗口完成直接发送、确认恰好一条「日程 · 名称」来源消息、一次宠物气泡和列表刷新；桌面锁、浮窗样式与强杀恢复按 [Eval 检查点](./evals/README.md) 手工验收。不以 Mock 或模拟结果替代真实 Provider。
...
- 验收：按 [Eval 检查点](./evals/README.md) 完成影响范围内的 Provider、窗口、桌面锁和宠物状态回归；重启后布局恢复；宠物状态切换流畅。

## 后续里程碑（保持 M2→M7 顺序）

- M2：时间统计 + 本地音乐播放器——**已实现**（v1.8 统计真实化 + v1.9 本地音乐播放器，ADR-0011）；M3：Codex CLI 嵌入（Claudian 式；对话 + focus-cli 白名单审计 + skills 透传，ADR-0007；plan mode/Diff/终端面板暂缓）；Claude Code 已由 ADR-0025 作为第二个真实 Provider 接入。M4：内置工作流引擎已冻结并退化为 Agent 日程工具（ADR-0019/0020/0021）；M5：Agent MVP/完善已落地（ADR-0022/0024/0025），后续按 Eval 回归；顺延：监督与软限制、实验性虚拟桌面（C# helper）、多 Agent 生态。
...
> OpenSpec 变更。
>
> 2026-08-14 实施进度（#128 一把做完 + 验收反馈轮 #129-#132）：C1-C4 及
> 后续 5 个反馈变更已全部实现、验收并归档（9 个 OpenSpec 变更，主规格 12 个
> capability）。UI 设计/维护文档：docs/ui-design.md（设计契约）与
> docs/ui-maintenance.md（维护流程）；事件订阅矩阵：
> docs/architecture/event-streams-v1.md。当前无进行中变更；未打 release tag。
