...
- M2：时间统计 + 本地音乐播放器——**已实现**（v1.8 统计真实化 + v1.9 本地音乐播放器，ADR-0011）；M3：Codex CLI 嵌入（Claudian 式；2026-08-06 已实现垂直切片：对话 + focus-cli 白名单审计 + skills 透传，ADR-0007；plan mode/Diff/终端面板/Claude Code 为后续）；M4：内置工作流引擎（精简 n8n；2026-08-07 已讨论方向：步骤列表 / 内部节点 / 手动+事件+定时 / 新浮窗 / 发完即走 / 专注结束+监督告警 / 3 模板 / 条件守卫；**未实施**）；M5：新的 Agent（外部 Agent 驱动的角色循环；2026-08-07 已锁定方向：内核驱动 / 事件+兜底 / 先单角色；Journal/Task 保持外接 skill 不内置；**未实施**）；后续顺延：监督与软限制、实验性虚拟桌面（C# helper）、多 Agent 生态。
