...
> 更新：2026-08-08（v1.10.5 已实现待验收：#59–#63，ADR-0018；前序 v1.10.4（#49–#58，ADR-0017）已实现待验收）。新对话请先读本页，再按需查阅 next-phase / requirements / ADR。
...
## 当前版本与已验证清单（v1.10.4 实施中；前序 v1.10.3.1/v1.10.3/v1.10.2/v1.10.1/v1.10/v1.9.1 + M4 v1 已实现待验收）
- v1.10.4（工作流 v2 重设计 + 白框/亮度修复 + 随机播放，实施中，需求 #49–#58，ADR-0017）：#49 WS_POPUP+外框重置客户区消除四周白边；#50 拖动亮度中心按客户区；工作流 v2=8 类节点/Agent 填空槽/分支多路“选项1..N”/允许成环+箭头/触发头徽标/无模板/自动保存/三栏 150/210/窗口 6×5/运行记录移设置页；#58 音乐随机播放第 4 模式。
